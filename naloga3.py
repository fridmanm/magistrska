...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 16 13:52:46 2016
...
#histogram dogodgov porazeljen po energijah
...
#deljenje dogodkov na dve območji energij 
...
for i in range(len(data)):      #zanka za filtriranje podatkov
    if log10E[i]<=5.1:          #določitev meje na roko iz histograma
        dec1.append(dec[i])
        ra1.append(ra[i])
    else:
        dec2.append(dec[i])
...
#Izris mape z dogodki pri nižjih energijah
...
#Izris mape z dogodki pri višjih energijah
...
#histogram dogodkov z nižjimi energijami po deklinaciji

dec1*=(np.pi/180)

bins = np.linspace(-1, 1, 50)
plt.hist(np.sin(dec1), bins, log=False)
plt.xlabel('$\sin{(dec)}$')
plt.ylabel('Število dogodkov')
plt.title('Dogodki z nižjimi energijami')
plt.xlim(-1,1)
plt.figure(figsize=(20,20))
plt.show()

# naključnih 10 map icecube podatkov z nižjimi energijami po phi
...
b=cm.Greys                                                           #      
b.set_under("w")                                                        #
hp.mollview(sum_rand_mapa, cmap=b, title='Normirana vsota naključnih map za nižje energije', cbar=True, xsize=1400)     #izris mape
hp.graticule(coord=('E'))                                               #
plt.show()
...
plt.figure()
plt.plot(Clp, label=' Power spectrum icecube\n podatkov z nižjimi energijami brez shot noise')
plt.legend(loc=1)
#plt.ylim([-6,-3])
#plt.xlim(0,100)
plt.xlabel('$\ell$')
...
plt.figure()
plt.plot(l,(C_l), label=' Povprečen power spectrum\n icecube podatkov (nizke E)')
plt.plot(ell,5*fit_Cl,label='Teoretična krivulja')
...
print('chi2(icecube low E) =',chi2(C_l,error,0))

#histogram dogodkov z višjimi energijami po deklinaciji

dec2*=(np.pi/180)

bins = np.linspace(-1, 1, 50)
plt.hist(np.sin(dec2), bins, log=False)
plt.xlabel('$\sin{(dec)}$')
plt.ylabel('Število dogodkov')
plt.title('Dogodki z višjimi energijami')
plt.xlim(-1,1)
plt.figure(figsize=(20,20))
plt.show()

# naključnih 10 map icecube podatkov z višjimi energijami po phi
...
plt.figure()
plt.plot(Clp2, label=' Power spectrum icecube\n podatkov z višjimi energijami brez shot noise')
plt.legend(loc=1)
#plt.ylim([-6,-3])
#plt.xlim(0,100)
plt.xlabel('$\ell$')
...
print('chi2(icecube high E) =',chi2(C_l2,error2,0))
